...
from Harmony import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
zchannel = {"@Harmony", "@Harmony_Support", "@Tws_Harmony", "@tepthonklaesh", "@PPYNY"}
...
async def mybot(): #Code by T.me/zzzzl1l
    ...
    if bot_name.startswith("مسـاعـد"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", "Harmony")
            ...
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_file("@BotFather", "Harmony/zilzal/logozed.jpg")
            await asyncio.sleep(3)
    ...
    try:
        if BOTLOG:
            Config.ZEDUBLOGO = await zedub.tgbot.send_file(
                ...
                buttons=[(Button.url("𝐬𝐨𝐮𝐫𝐜𝐞 𝐭𝐞𝐩𝐭𝐡𝐨𝐧 🇵🇸", "https://t.me/Harmony"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def load_plugins(folder, extfolder=None):
    """
    Load plugins from the specified folder without any VPS checks.
    """
    if extfolder:
        path_pattern = f"{extfolder}/*.py"
        plugin_path = extfolder
    else:
        path_pattern = f"Harmony/{folder}/*.py"
        plugin_path = f"Harmony/{folder}"
    ...
    for file_path in files:
        ...
        plugin_name = path_obj.stem  # اسم الملف بدون .py

        try:
            for attempt in range(5):  # محاولة تحميل حتى 5 مرات
                try:
                    load_module(plugin_name, plugin_path=plugin_path)
                    success += 1
                    ...
                    install_pip(e.name)
                    if attempt == 4:  # بعد آخر محاولة
                        failure.append(plugin_name)
        except Exception as e:
            failure.append(plugin_name)
            LOGS.info(f"لا يمكنني تحميل {plugin_name} بسبب الخطأ {e}\nمجلد القاعدة {plugin_path}")

    # إرسال تقرير لو كان هذا تحميل خارجي
    if extfolder:
        if not failure:
            failure.append("None")
        await zedub.tgbot.send_message(
            BOTLOG_CHATID,
            f'Your external repo plugins have imported\n**No of imported plugins:** `{success}`\n**Failed plugins:** `{", ".join(failure)}`'
        )
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photozed = await zedub.upload_file(file="zedthon/malath/Harmony.jpg")
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    await load_plugins(folder="Harmony", extfolder=cfolder)
