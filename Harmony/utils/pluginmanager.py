...
from Harmony import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"Harmony/plugins/{shortname}.py")
        checkplugins(path)
        name = "Harmony.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"Harmony/plugins/{shortname}.py")
            name = f"Harmony.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"Harmony.plugins.{shortname}"] = mod
        LOGS.info(f"Successfully imported {shortname}")
